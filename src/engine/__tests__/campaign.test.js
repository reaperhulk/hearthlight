...
function build(state, slot, building) {
  return command(state, { type: "build", slot, building });
}
function start() {
  return startGame(freshGame(), "first", 42);
}
...
describe("the village campaign", () => {
  ...
  it("wave generation is independent of building choices and uses the real map", () => {
    ...
    for (const town of ["first", "meadow", "marsh", "ridge"]) {
      ...
      for (let night = 1; night <= 15; night++)
        expect(
          makeWave(town, night, 42).every((e) => e.lane < lanes && e.at >= 2),
        ).toBe(true);
    }
  });
  ...
  it("specializations, repair and relocation retain meaningful budgets", () => {
    ...
    expect(state.round.slots[0].building.hp).toBe(
      maxHp(state.round.slots[0].building, "keeper"),
    );
    expect(state.round.slots[0].building.hp).toBe(135);
    expect(
      command(state, { type: "upgrade", slot: "0-0", branch: "thorns" }),
    ).toBe(state);
  ...
  it("legacy progress is preserved and incompatible active rounds are archived", () => {
    const state = migrateGame({
      saveVersion: 1,
      embers: 31,
      meta: { stoneFoundations: 2, swiftWarden: true },
      bestNights: 12,
      totalRounds: 5,
      round: { phase: "night" },
    });
