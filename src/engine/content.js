...
export const ENEMIES = {
  ...
  runner: {
    ...
    interval: 1.25,
    color: "#dcc18c",
    description: "Fast, fragile, and dangerous on an unguarded road.",
  },
  mist: {
    name: "Veil bearer",
    hp: 30,
    ...
    color: "#8dbcbf",
    description:
      "Nearby enemies take 35% less tower damage. Burst interrupts its veil.",
  },
};
...
export const KITS = {
  keeper: {
    name: "Hearthkeeper",
    cost: 0,
    detail: "A balanced beginning. Two lantern bursts each night.",
  },
  mason: {
    name: "Stone & timber",
    cost: 8,
    detail: "Walls have 25 extra health. Repairs cost 2 less Glow.",
  },
  ranger: {
    name: "The wandering light",
    cost: 12,
    detail: "The Warden moves 40% faster and hits for 2 more damage.",
  },
  gardener: {
    name: "Seeds of tomorrow",
    cost: 12,
    detail: "Start with 10 more Glow. Farms produce 5 extra at dawn.",
  },
};
...
export const BLESSINGS = {
  kindle: {
    name: "Wildfire",
    detail: "Lantern bursts also burn enemies for 12 damage over 3 seconds.",
  },
  chain: {
    name: "Kindred light",
    detail:
  ...
  reserves: {
    ...
    detail: "One extra lantern burst each night.",
  },
  salvage: {
    name: "What remains",
...
export function createMap(town = "first") {
  ...
  const slots = lanes.flatMap((lane) =>
    [0.3, 0.48, 0.64, 0.79].map((progress, index) => {
      ...
      return {
        ...
        index,
        progress,
        x: p.x + Math.cos(lane.angle + Math.PI / 2) * offset,
        y: p.y + Math.sin(lane.angle + Math.PI / 2) * offset,
        building: null,
      };
    }),
  );
...
export function routePoint(lane, progress, town) {
  ...
  return {
    x:
      0.5 +
      Math.cos(lane.angle) * radius +
      Math.cos(lane.angle + Math.PI / 2) * curve,
    y:
      0.5 +
      Math.sin(lane.angle) * radius +
      Math.sin(lane.angle + Math.PI / 2) * curve,
  };
}
...
export function makeWave(town, night, seed) {
  const count =
    town === "first" && night <= 3
      ? [3, 6, 9][night - 1]
      : Math.min(120, 5 + night * 3 + (town === "ridge" ? 3 : 0));
  const lanes = mapLanes(town);
  const rng = randomSequence(
    seed + night * 7919 + Object.keys(TOWNS).indexOf(town) * 433,
  );
  const enabled =
    town === "first" ? Math.min(night, lanes.length) : lanes.length;
  return Array.from({ length: count }, (_, index) => {
    ...
    return {
      id: `${night}-${index}`,
      type,
      lane: index < enabled ? index : Math.floor(rng() * enabled),
      at:
        2 +
        index * (town === "first" ? 2.5 : 1.65) +
        Math.round(rng() * 5) / 10,
    };
  });
}
